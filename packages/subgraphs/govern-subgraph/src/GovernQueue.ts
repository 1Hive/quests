...
export function handleScheduled(event: ScheduledEvent): void {
  ...
  let payload = createPayload(event.params.containerHash);
  let container = createContainer(event.params.containerHash);
...
export function handleExecuted(event: ExecutedEvent): void {
  let container = loadOrThrowContainer(event.params.containerHash);
...
export function handleChallenged(event: ChallengedEvent): void {
  ...
  let resolver = loadConfig(queue.config).resolver;
...
export function createContainer(containerHash: Bytes): Container {
...
export function loadOrThrowContainer(containerHash: Bytes): Container {
  let ContainerId = containerHash.toHex();
  // Create container
  let container = Container.load(ContainerId);
  if (container === null) {
    throw new Error("Container not found with hash :" + containerHash);
  }
  return container!;
}

function createPayload(containerHash: Bytes): Payload {
